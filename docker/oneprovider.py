...
ONP_RELEASES_SOURCES_PATH = "_build/default/rel/op_panel/releases/RELEASES"
...
ONP_VERSION_FILE_PACKAGES_PATH = "/etc/op_panel/deployed-version.json"
ONP_RELEASES_PACKAGES_PATH = "/usr/lib/op_panel/releases/RELEASES"

# op_worker config / Onezone lookup (when deployed-version.json is missing)
OP_WORKER_AUTOGENERATED_CONFIG_PATH = "/etc/op_worker/autogenerated.config"
OP_WORKER_PROVIDER_ROOT_TOKEN_PATH = "/etc/op_worker/provider_root_token.json"
...
Resuming Oneprovider in version {current_version} ...
"""
...
def infer_previous_version() -> str:
    ...
    Lookup order:
      1. Local version file (deployed-version.json) – written by us on every
         successful start since 25.1.
      2. Onezone REST API – the cluster record always stores the version that
         was last reported to Onezone; works for any prior release.
      3. Hardcoded fallback (ONP_DEFAULT_ASSUMED_VERSION) with a 30 s warning
         sleep, giving the operator a chance to abort.
    """
    version = read_version_from_file()
    if version:
        return version

    version = infer_version_from_onezone()
    if version:
        return version
...
def infer_version_from_onezone() -> Optional[str]:
    ...
    if not oz_domain:
        log(
            "Cannot query Onezone for previous version: "
            f"oz_domain not found in {OP_WORKER_AUTOGENERATED_CONFIG_PATH}"
        )
        return None

    cluster_id, token = read_provider_credentials()
    if not cluster_id or not token:
        log(
            "Cannot query Onezone for previous version: "
            f"provider credentials not found in {OP_WORKER_PROVIDER_ROOT_TOKEN_PATH}"
        )
        return None
    ...
    try:
        ...
        version = data["workerVersion"]["release"]
...
def read_provider_credentials() -> Tuple[Optional[str], Optional[str]]:
    ...
    try:
        with open(OP_WORKER_PROVIDER_ROOT_TOKEN_PATH, "r") as f:
            data = json.load(f)
        return data.get("provider_id"), data.get("root_token")
...
def fetch_cluster_info(oz_domain: str, cluster_id: str, token: str) -> Optional[str]:
    ...
    """
    base = f"https://{oz_domain}" if not oz_domain.startswith("http") else oz_domain
    url = f"{base.rstrip('/')}/api/v3/onezone/clusters/{cluster_id}"
    try:
        r = requests.get(
            url,
