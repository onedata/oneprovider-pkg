...
###-------------------------------------------------------------------
### This script is called when the Oneprovider docker receives a termination
### signal. It makes sure that all system services of the Oneprovider instance
...
EOF
    ...
    dispatch-log "WARNING: detected a forced attempt to stop the Oneprovider service!"
...
'/root/oneprovider-stop-graceful.sh' script BEFORE terminating the container.
Depending on the installation type, there are several ways to do that:

    * onedatify - in the recent version (21.02.2+) of this tool, it is done
      automatically when using the 'onedatify stop' command. If you installed
      onedatify in the past, it must be upgraded. Download the package
      matching your OS type and simply install it with the package
      manager (apt/yum etc.) over the previous installation:

        * https://packages.onedata.org/onedatify_21.02.2.systemd_amd64.deb
        * https://packages.onedata.org/onedatify-21.02.2.systemd-1.x86_64.rpm
